...
class Flickr8kDataset(Dataset):
    ...
    def __init__(self, root: str | Path, split: str = "train", image_size: int = 224):
        ...
        self.split_images = [ln.strip() for ln in self.split_path.read_text(encoding="utf-8").splitlines() if ln.strip()]
        ...
        for ln in self.captions_path.read_text(encoding="utf-8", errors="ignore").splitlines():
            ...
            if img_name in self.split_images_set:
    ...
    def __getitem__(self, idx: int) -> Flickr8kSample:
        ...
        return Flickr8kSample(pixel_values=pixel_values, text=caption, image_name=img_name)
